...
int main(int argc, char const *argv[])
{
    ...
    while(1){
        ...
        if (string_equal(get_input,"insert"))
        {
            ...
            if(id >= 1){
                int signal = close_task(board, id);
                if(signal == -1) printf("There is no task with id %d being executed", id);
            }else printf("%s is not a valid id\n", get_input);
            ...
            if(id >= 1){
                scanf("%s", get_input);
                int signal = change_name(board, id, get_input);
                if(signal == -1) printf("The id %d was not found in the Doing list\n", id);
            }
            else printf("%s is not a valid id\n",get_input);
        }
